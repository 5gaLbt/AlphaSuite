...
def merge_df(trained_df, df):
    # Rename columns before concatenating
    trained_df = trained_df.rename(columns={'value': 'trained_value'})
    df = df.rename(columns={'value': 'passthrough value'})

    # Merge on 'name' column
    return pd.merge(trained_df, df, on='name')


def run_backtest_trained_vs_passthrough(portfolio_name, tickers, strategy_type, start_date, end_date):
    ...
    print(f"====== PASSTHROUGH Model BACKTEST {portfolio_name} ========")
    result = run_pybroker_portfolio_backtest(
        ...
        commission_cost=0.0)
    compare_df = merge_df(trained_result.metrics_df, result.metrics_df)
    print("\n")
...
if __name__ == "__main__":
    ...
    strategy_types = ['structure_liquidity', 'donchian_breakout']
    ...
    for strategy_type in strategy_types:
        for i, tickers in enumerate(tickers_list):
            ...
            df =run_backtest_trained_vs_passthrough(
                portfolio_name=portfolio_name,
                tickers=tickers,
                strategy_type=strategy_type,
                start_date=start_date,
